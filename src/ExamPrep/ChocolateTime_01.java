...
public class ChocolateTime_01 {
    public static void main(String[] args) {
        ...
        chocolateMap.put("Milk", 0);
         ...
         long producedChocolateTypes = chocolateMap.entrySet()
                 .stream()
                 .filter(entry -> entry.getValue() != 0)
                 .count();

         if (producedChocolateTypes == 3) {
             System.out.println("It's a Chocolate Time. All chocolates types are prepared.");
             ...
             chocolateMap.entrySet()
                     .stream()
                     .filter(entry -> entry.getValue() != 0)
                     .forEach(entry -> System.out.printf("# %s Chocolate --> %d\n", entry.getKey(), entry.getValue()));
